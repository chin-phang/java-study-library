...
1. Why can't you get "exactly one worker handles this message, and another
   worker picks it up if the first one dies" for free out of a Kafka consumer
   group the way you can out of a RabbitMQ queue?
2. What is the one piece of state that acknowledgement, prefetch,
   dead-lettering and poison-message handling are all different views of?
3. A Kafka consumer redoes work by seeking its offset backward. What is the
   structurally equivalent operation in RabbitMQ, and why does it only ever
   apply to one message at a time instead of a range?
4. Why does adding a second consumer to a single RabbitMQ queue break
   ordering, when adding a second consumer group to a Kafka topic doesn't
   touch ordering at all?
5. A RabbitMQ stream and a RabbitMQ quorum queue are both replicated via
   Raft. Name two consumer-visible behaviours that differ between them
   despite that, and explain why replication being the same mechanism
   doesn't make the two the same abstraction.
6. Why does `rabbitmqadmin get ... ackmode=ack_requeue_false` against an
...
7. A consumer acks a message, then crashes before the ack frame reaches the
   broker. What does the broker do, and what does that imply about
   idempotency that has nothing to do with how careful the consumer's code
   is?
