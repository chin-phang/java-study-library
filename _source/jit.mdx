...
- **Startup and the cold path.** Tiered compilation improves warmup, but it
  ...
  Leyden's AOT cache (`-XX:AOTCache`, shipping since JDK 24 and extended in 25
  and 26), and GraalVM native images all address this differently.
...
1. Why does inlining matter far more than the call overhead it removes?
2. A hot method is 400 bytes of bytecode. What does C2 do, and what is the
   knock-on effect on its caller?
3. Define monomorphic, bimorphic and megamorphic call sites, and say which
   transition costs the most.
4. What is profile pollution, and why can a shared utility method make an
   unrelated call site slower?
5. Explain deoptimisation. What is an uncommon trap, and what does a deopt loop
   look like from outside?
6. List four distinct ways a hand-written benchmark can produce a wrong number,
   and say how JMH defeats each.
7. Your service is slow for the first 200 requests after every deploy. Give two
   legitimate responses and one that merely hides it.
8. What is safepoint bias and why can it make a profiler blame the wrong method?
