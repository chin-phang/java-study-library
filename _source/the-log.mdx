...
tier: foundational
prerequisites: [mvcc]
unlocks: [replication, cdc-and-outbox, event-sourcing, kafka-internals, crash-recovery]
...
- PostgreSQL docs chapter 29 (Reliability and the WAL) and 27 (Replication).
...
1. Why does writing a log record *and then* a data page make commits faster than
   writing the data page directly?
2. What is an LSN, and what is its exact counterpart in Kafka?
3. Explain a PostgreSQL replication slot as a consumer offset. What does each
   guarantee to its writer, and what happens when the reader dies?
4. Why does an abandoned replication slot cause both disk exhaustion *and* table
   bloat? (Two independent mechanisms.)
5. What does a checkpoint trade against what? Name the two settings that move
   the dial and which direction each goes.
6. What is a compacted Kafka topic equivalent to in database terms, and what
   does replaying it from offset 0 produce?
7. Why can't you answer "what is account 42's balance" from a log alone, and
   what do you need to build?
8. Zero-copy is disabled in two situations. Name them and explain the observable
   symptom.
