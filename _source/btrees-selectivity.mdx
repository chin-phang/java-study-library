...
tier: foundational
prerequisites: []
unlocks: [query-planning, composite-indexes, mvcc, partitioning, covering-indexes]
...
1. Why does an index on a boolean column help for one value and not the other?
   Express the answer in terms of page reads, not "selectivity".
2. Explain the crossover point between index scan and sequential scan. Which two
   configuration parameters move it, and which direction?
3. Why does a bitmap heap scan exist, given index scans and sequential scans
   already do?
4. `WHERE a = ? AND b > ? ORDER BY b` — give the correct composite index and
   explain what breaks if you reverse the columns.
5. An index-only scan reports `Heap Fetches: 48000`. What happened, and what
   would you run?
6. Estimated rows 12, actual rows 400,000, and a nested loop join. Explain the
   causal chain, and name two ways to fix the estimate rather than the plan.
7. Why is `random_page_cost = 4.0` a bad default on SSD, and what is the
