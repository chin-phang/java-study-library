...
1. State the three outcomes of a network call and explain why the third cannot
   be engineered away.
2. Why is a unique constraint the right mechanism for idempotency, and what
   exactly goes wrong with `if (!exists) insert`?
3. Why must the dedup marker and the side effect share a transaction? Describe
   the specific failure if they don't.
4. Who generates the idempotency key and when? Give two wrong answers and why
   each fails.
5. Same key, different request body. What should the API return, and what is the
   harm in silently returning the stored response?
6. A retry arrives while the first attempt is still executing. Give three valid
   responses and one invalid one.
7. Distinguish "idempotent" from "safe to execute concurrently". Give an
   operation that is one but not the other.
8. Your idempotency implementation is correct and the acquirer still shows a
