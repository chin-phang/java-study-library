...
unlocks: [distributed-locks, saga-pattern, connection-pooling, retry-design]
questions: [data/transactions-mvcc#q35, data/transactions-mvcc#q36, java/concurrency#q55, java/concurrency#q59]
...
1. State the four Coffman conditions. Which one does lock ordering break, and
   why is that usually the cheapest to attack?
2. Write the account-transfer deadlock, then fix it with consistent ordering.
   What happens if two accounts can have the same ID?
3. Why does an unindexed `UPDATE ... WHERE merchant_id = ?` risk a deadlock that
   an indexed version of the identical statement doesn't?
4. Explain precisely why the JVM can hang forever on a `synchronized` deadlock
   while a database transaction cannot.
5. A `DELETE` matches zero rows. Under what circumstances can it still block a
   later `INSERT`, and why?
6. Your application logs `deadlock detected` as an ERROR and returns a 500 to
   the caller. What has gone wrong with the response, given that the database
   already did its job correctly?
7. Name two disciplines that prevent Java-level lock deadlocks beyond ordering
   alone.
8. Distinguish deadlock from livelock from starvation, and give the
