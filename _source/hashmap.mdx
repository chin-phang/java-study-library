...
tier: foundational
prerequisites: []
unlocks: [concurrent-collections, equals-hashcode, collection-sizing, caching]
...
([Q116](/docs/data/microservices-data#q116) in the design bank's persistence
material) is where this bites in real codebases. Business key, or
...
1. Why does `HashMap` use `(n-1) & hash` rather than `hash % n`, and what
   problem does that create?
2. Explain what `h ^ (h >>> 16)` fixes. Give a concrete key type where omitting
   it would be catastrophic.
3. A bucket reaches 8 entries. Why is that essentially impossible by chance, and
   what does the JDK do about it?
4. Why does treeification require the table to be at least 64 slots?
5. You add an object to a `HashSet`, mutate one field, and `contains` returns
   false for the same reference. Explain precisely where the object is.
6. Describe the Java 8 resize split. Why does it need no rehashing, and what did
   it fix from Java 7?
7. `new HashMap<>(1000)` — how many entries can you insert before it resizes?
8. Estimate the memory for `Map<Integer,Integer>` with 5 million entries, and
