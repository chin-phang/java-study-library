...
unlocks: [isolation-levels, vacuum-and-bloat, long-transactions, replication-lag, write-skew]
...
- PostgreSQL docs, chapter 13 (Concurrency Control) and 25.1 (Routine
  Vacuuming) — read 25.1 in full once; it is short and it is the operational
  manual for this page.
...
1. Explain precisely why `UPDATE accounts SET balance = ...` writes to every
   index on the table, and the two conditions under which it doesn't.
2. A table holds 400,000 rows and occupies 14 GB. `VACUUM` completes
   successfully and changes nothing. Give the most likely cause and the query
   you'd run to confirm it.
3. Why can't `count(*)` be O(1) in PostgreSQL, and why is that a consequence of
   MVCC rather than an implementation shortcoming?
4. What is the visibility map, which operation maintains it, and which query
   plan silently degrades without it?
5. Your colleague's idle transaction is on an unrelated table. Explain why it
   still causes bloat in the payments table.
6. What is transaction ID wraparound, why is it a hard outage rather than a
   slowdown, and which single number would you graph to prevent it?
7. A `@Transactional` service method makes an HTTP call to a payment provider
   with a 30-second timeout. Describe the database-wide consequence under load.
8. How does InnoDB's approach differ, and what is the equivalent symptom to
   PostgreSQL bloat?
