...
1. Why does `nums.parallelStream().reduce(0, (a, b) -> a - b)` produce a
...
6. Two teams share a JVM. Team A adds `.parallel()` around a stream that
   ...
   the connection.
7. `Stream.iterate(1, i -> i + 1)` (two-arg form) and `IntStream.range(0,
