...
worse — it copies the map to every child. Java 21 previews `ScopedValue` as the
replacement: immutable, bound for a dynamic extent, automatically unbound, no
copying ([Q91](/docs/java/modern-java#q91)).
...
`StructuredTaskScope` (preview in 21) gives concurrent tasks block scope:
...
(Preview API: `--enable-preview --release 21`.)
...
- JEP 444 (Virtual Threads) — the motivation section is the best short
  ...
  removal, JEP 453 for structured concurrency, JEP 446 for scoped values.
