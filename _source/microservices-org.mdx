...
1. State, in one sentence, what microservices buy — then name three benefits
   commonly attributed to them that are available without them.
2. An organisation has one product team and seven services. Name the quadrant,
   predict what its deployment record will look like, and say why each
   individual split was probably defensible when it was made.
3. Why is the benefit of microservices conditional while the costs are
   unconditional, and what follows from that asymmetry for a team deciding
   whether to split?
4. Give the four preconditions, and for each one name something you could check
   this week rather than argue about.
5. "We're too small for microservices, so we don't need to worry about
   boundaries." Explain what is right and what is wrong in that sentence.
6. Amazon Prime Video's monitoring component and Istio's control plane both
   merged back. Explain why only one of them is evidence for the argument on
   this page.
7. Google's Service Weaver let you write a logical monolith and decide the
   deployment topology at runtime. Explain why that is a plausible idea, and why
   it does not resolve the question this page asks.
8. You have concluded that two services should be merged. Give the order of
   operations, say which step is irreversible, and give the framing you would
   use with the engineer who designed the split.
