...
unlocks: [multi-region-replication, schema-evolution]
...
<SelfCheckItem n={1} where="§3 The core model, part one: ISR and the high watermark.">

A partition's leader has a log end offset of 500; the high watermark is
480. The leader dies right now. What is guaranteed about offsets 481–500,
and why doesn't a client ever notice they existed?

</SelfCheckItem>

<SelfCheckItem n={2} where="§3 The core model, part one: ISR and the high watermark.">

`acks=all` is set, and a producer's write is acknowledged. Under what
...
<SelfCheckItem n={3} where="§3 The core model, part one: ISR and the high watermark; §4 The core model, part two: consumer groups and rebalancing.">

Two different problems are both called "rebalancing risk" in casual
...
<SelfCheckItem n={4} where="§4 The core model, part two: consumer groups and rebalancing.">

Why does cooperative incremental assignment change *which* partitions
move during a rebalance, without changing *how many* members are in the
group afterward?

</SelfCheckItem>

<SelfCheckItem n={5} where="§6 Adjacent guarantees, with consumer lag as an operational signal.">

A consumer group's lag spikes at the same moment the topic's log-end
offset spikes. Is this a consumer problem or a producer problem, and what
is the distinguishing observation?

</SelfCheckItem>

<SelfCheckItem n={6} where="§4 The core model, part two: consumer groups and rebalancing, static membership.">

Static membership prevents a rebalance on a routine rolling restart. What
new failure mode does it introduce, and why is it usually the better
trade anyway?

</SelfCheckItem>

<SelfCheckItem n={7} where="§6 Adjacent guarantees, with CAP.">

Explain Kafka's CAP position using `min.insync.replicas` and
