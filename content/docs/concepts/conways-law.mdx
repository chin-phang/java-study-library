...
unlocks: [microservices-org, platform-engineering, service-decomposition]
...
<SelfCheckItem n={1} where="§2 What the law actually says.">

State Conway's law and explain, in terms of design decisions and communication
paths, why "constrained" is the accurate word rather than "influenced".

</SelfCheckItem>

<SelfCheckItem n={2} where="§4 The core model: coordination cost decides which boundaries survive.">

Two teams own four services. Predict which boundaries will erode and which will
hold, and give the cost argument that makes it a prediction rather than an
observation after the fact.

</SelfCheckItem>

<SelfCheckItem n={3} where="§4 You cannot enforce a boundary with discipline against its own cost structure.">

Your team agrees a module boundary in review and it erodes anyway over six
months, with nobody behaving badly. Explain how, and name two mechanisms that
would have held it.

</SelfCheckItem>

<SelfCheckItem n={4} where="§3 Organisational structure out-predicts every code metric.">

Organisational metrics out-predicted code churn, complexity, coverage and
...
<SelfCheckItem n={5} where="§5 Measuring your own congruence; §9 Lab 2.">

Define socio-technical congruence, and describe how you would approximate it
from a git repository and a `CODEOWNERS` file.

</SelfCheckItem>

<SelfCheckItem n={6} where="§6 The inverse manoeuvre, the three interaction modes.">

Two teams have had a standing weekly sync for a year. Name the Team Topologies
interaction mode they are in, say what that implies about the boundary between
them, and describe what you would check next.

</SelfCheckItem>

<SelfCheckItem n={7} where="§7 The causation runs both ways.">

Why does the Inverse Conway Manoeuvre fail when a reorganisation happens
without extraction — and what does that imply about sequencing the two?

</SelfCheckItem>

<SelfCheckItem n={8} where="§8 A shared library is an unintended communication path; §9 Lab 5.">

A team proposes moving a shared domain type into the common library "to stop
