...
`synchronized` blocks can block carriers on Java 21 specifically, which is worth
auditing before you enable them there ([Q62](/docs/java/concurrency#q62)).

That last one has since been fixed: **JEP 491 removed `synchronized` pinning in
JDK 24**, so from 24 onward a virtual thread blocked on a monitor unmounts and
releases its carrier, and migrating `synchronized` to `ReentrantLock` purely for
scalability is no longer necessary. Native frames and a few JVM-internal
critical sections still pin, so the audit shrinks rather than disappears.
...
- JEP 444 (Virtual Threads) and the Structured Concurrency JEP — the motivation
  sections are the best writing on why thread-per-request came back. Read the
  *current* preview rather than the first: structured concurrency has re-previewed
  six times (JEP 453 in JDK 21 through JEP 525 in JDK 26) and the API was
  reshaped around `StructuredTaskScope.open()` on the way, so the early JEPs show
  code that no longer compiles. The motivation sections have not changed.
...
<SelfCheckItem n={1} where="§2 Little's Law.">

State Little's Law and use it to explain why a service with 50 threads
cannot serve 500 rps at 200 ms.

</SelfCheckItem>

<SelfCheckItem n={2} where="§1 The problem it exists to solve; §2 Little's Law.">

A downstream slows from 40 ms to 4 s. CPU stays near zero and every endpoint
returns 503. Explain the mechanism precisely.

</SelfCheckItem>

<SelfCheckItem n={3} where="§3 The ThreadPoolExecutor trap; §7 Lab 1 — prove maximumPoolSize is ignored.">

`new ThreadPoolExecutor(10, 500, ..., new LinkedBlockingQueue<>())` — what is
the effective maximum pool size, and why?

</SelfCheckItem>

<SelfCheckItem n={4} where="§3 The ThreadPoolExecutor trap.">

What is wrong with `Executors.newCachedThreadPool()` under sustained load?

</SelfCheckItem>

<SelfCheckItem n={5} where="§4 Bounded queues, and why unbounded ones are a lie.">

Give three distinct harms caused by an unbounded work queue.

</SelfCheckItem>

<SelfCheckItem n={6} where="§4 Bounded queues, and why unbounded ones are a lie; §7 Lab 4 — rejection policies.">

When is `CallerRunsPolicy` the right choice, and what is its sharp edge?

</SelfCheckItem>

<SelfCheckItem n={7} where="§6 What virtual threads change — and what they don't.">

You migrate to virtual threads and remove your thread pools. What must you
add, and what happens if you don't?

</SelfCheckItem>

<SelfCheckItem n={8} where="§8 Leading on this; §7 Lab 6 — thread dump forensics.">

Your pool metrics show rising queue depth and flat CPU. What does that tell
