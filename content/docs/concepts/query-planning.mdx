...
unlocks: [statistics-tuning, plan-stability, composite-indexes]
...
<SelfCheckItem n={1} where="§4 Why one bad estimate ruins a whole plan.">

A plan shows `rows=12` against `actual rows=412006`. Name the separate decisions that one number corrupted, and say which of them you can fix.

</SelfCheckItem>

<SelfCheckItem n={2} where="§3 Multiple predicates are combined by multiplying their selectivities.">

Why does `WHERE city = ? AND country = ?` underestimate even when statistics were refreshed this morning? Name the assumption and the remedy.

</SelfCheckItem>

<SelfCheckItem n={3} where="§3 How a row estimate is actually computed.">

Give the arithmetic for an equality estimate on a value that is *not* in the MCV list, and explain why that formula is dangerous on a skewed column.

</SelfCheckItem>

<SelfCheckItem n={4} where="§4 Nested loop is the sharp edge.">

Why does a nested loop punish a bad estimate hardest, when hash and merge joins survive the same error?

</SelfCheckItem>

<SelfCheckItem n={5} where="§5 Join ordering, and the limits the planner sets on itself.">

A fifteen-table reporting query returns different plans on different runs with no data change. What is happening, and what are your two options?

</SelfCheckItem>

<SelfCheckItem n={6} where="§6 A materialisation fence.">

Before PostgreSQL 12 a CTE was an optimisation fence. What replaced that behaviour, and in which two situations do you still want the fence?

</SelfCheckItem>

<SelfCheckItem n={7} where="§7 The plan you get is not always the plan for your parameters.">

A prepared statement is fast five times and slow on the sixth. Explain the mechanism precisely, and rank the fixes.

</SelfCheckItem>

<SelfCheckItem n={8} where="§10 Treat hints as debt with an expiry date, and Own statistics as a platform concern.">

Someone proposes adding `enable_nestloop = off` to the application's session setup because it resolved an incident. Give the argument against, and say what you would do instead.
