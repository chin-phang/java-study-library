...
```mermaid
flowchart TD
  A["Thread 1: data = 42"] --> B["Thread 1: volatile ready = true"]
  B -.->|happens-before| C["Thread 2: reads ready, sees true"]
  C --> D["Thread 2: reads data, sees 42"]
```
...
```mermaid
flowchart TD
  A["submit task"] --> B{"below corePoolSize?"}
  B -->|yes| C["start a core thread"]
  B -->|no| D{"queue has room?"}
  D -->|yes| E["enqueue, wait for a thread"]
  D -->|no| F{"below maximumPoolSize?"}
...
```mermaid
flowchart TD
  A["virtual thread mounted on a carrier"] --> B{"what blocks it?"}
  B -->|"JDK blocking call: socket, sleep, queue"| C["continuation unmounts, stack parked on heap"]
  C --> D["carrier freed for another virtual thread"]
  D --> E["remounts on any carrier when ready"]
  B -->|"synchronized block or native call"| F["pinned: cannot unmount"]
  F --> G["carrier stays blocked; pin them all and the app deadlocks"]
```
