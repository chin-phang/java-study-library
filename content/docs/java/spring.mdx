...
```mermaid
flowchart TD
  A["caller"] --> B["proxy: transaction interceptor"]
  B --> C["target bean: outer method"]
  C --> D["this.doWork() — internal call"]
  D --> E["target bean: doWork body runs"]
  E --> F["proxy never crossed, so no transaction"]
```
