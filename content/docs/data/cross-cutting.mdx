...
```mermaid
flowchart TD
  A["one local transaction: business change + outbox insert"] --> B["outbox row with published_at null"]
  B --> C{"relay"}
  C -->|polling| D["SELECT unpublished FOR UPDATE SKIP LOCKED, publish, mark"]
  C -->|CDC| E["Debezium tails the WAL for outbox inserts"]
  D --> F["broker, keyed by aggregate id to preserve per-aggregate order"]
  E --> F
  F --> G["at-least-once, so the consumer dedupes on event id"]
```
