...
```mermaid
flowchart TD
  P["publisher: publish with a routing key"] --> X["exchange"]
  X --> B["bindings, matched by the exchange type"]
  B --> Q1["queue A"]
  B --> Q2["queue B"]
  Q1 --> C1["consumer"]
  Q2 --> C2["consumer"]
...
```mermaid
flowchart TD
  M["main queue"] --> N["consumer nacks with requeue=false"]
  N --> A{"x-death count still below N?"}
  A -->|yes| R["retry queue: TTL set, no consumer"]
  R -->|"TTL expires, its DLX routes back"| M
  A -->|no| D["dead-letter queue: monitored, replayable"]
```
