...
```mermaid
flowchart TD
  A["consumers send JoinGroup to the group coordinator"] --> B["one member becomes leader, computes the assignment"]
  B --> C["coordinator distributes it via SyncGroup"]
  C --> D["each partition owned by exactly one consumer"]
  D --> E{"more consumers than partitions?"}
  E -->|yes| F["the surplus consumers sit idle"]
  E -->|no| G["every consumer owns at least one partition"]
```
...
```mermaid
flowchart TD
  L["leader: log end offset, say 105"] --> F["followers fetch from the leader, exactly like consumers"]
  F --> I["a follower within replica.lag.time.max.ms stays in the ISR"]
  I --> H["high watermark = highest offset held by every ISR member, say 103"]
  H --> C["consumers may read up to 103 only"]
  H --> U["104-105 not yet replicated, so not yet readable"]
  C --> E["leader fails: new leader from the ISR already holds 103, no acked data lost"]
```
