...
```mermaid
flowchart TD
  subgraph AG ["Order aggregate — one transaction"]
    direction TB
    R["Order (root): only entry point"] --> L1["OrderLine"]
    R --> L2["OrderLine"]
  end
  AG --> CID["holds CustomerId, not Customer"]
  CID -.->|"eventual consistency"| CU["Customer aggregate"]
```
