...
export function SelfCheckItem({
  ...
  n: number;
  /** Where on this page the model is built — a section name, not an answer. */
  where: string;
  children: ReactNode;
}) {
  // counted, so the page-level expand-all control appears on concept pages too
  const [open, setOpen] = useCollapsible(undefined, true);
